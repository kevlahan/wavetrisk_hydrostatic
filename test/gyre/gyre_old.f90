module gyre_mod
  ...
  real(8) :: FrBC, Re, delta_M, Rd, eta_gyre
  ...
  ! Dimensional scaling
  real(8) :: Ldim, Hdim, Udim, Tdim

  ! Dimensional parameters
  real(8) :: delta_M_star, c_star, g_star, Rd_star, Fr, U_Sv_star, nu_star, beta_star, omega_star, f0_star

  ! Non-dimensional quantities
  real(8) :: U_BC, H, Lx, U_Sv, nu, c, g, f0, Ro, csq, c_p
...
contains
 ...
 subroutine wrap_lonlat(s, t)
  ...
  subroutine read_test_case_parameters(filename)
      ...
      read(fid,*) varname, FrBC    
      read(fid,*) varname, Re      
      read(fid,*) varname, delta_M 
      if (rank.eq.0) then
...
program gyre
  ...
  Rd       = Re * delta_M**2/FrBC     ! Rossby radius
  eta_gyre = FrBC * sqrt(FrBC/Rd/Re) ! Variation in ocean height east-west due to gyre flow

  ! Dimensional scaling
  Ldim = Lx_star  ! Horizontal length scale 
  Hdim = H_star   ! Vertical length scale
  Udim = U_BC_star ! Velocity scale is boundary layer speed
  Tdim = Ldim/Udim ! Time scale

  ! Dimensional parameters
  delta_M_star = delta_M * Ldim                   ! Width of boundary current
  c_star       = U_BC_star / FrBC                 ! Wave speed
  g_star       = c_star**2 / H_star               ! Gravity
  Rd_star      = Rd * Ldim                        ! Rossby radius 
  Fr           = FrBC * delta_M_star/Lx_star      ! Froude number
  U_Sv_star    = U_BC_star * delta_M_star/Lx_star ! Gyre velocity
  nu_star      = U_BC_star * delta_M_star / Re    ! Viscosity
  beta_star    = nu_star / delta_M_star**3        ! Beta
  omega_star   = radius_star * beta_star/sqrt(2.0_8)   ! Rotation rate based on 45 degrees north
  f0_star      = 2.0*omega_star

  ! Non-dimensional quantities
  U_BC       = U_BC_star / Udim       ! Velocity of boundary layer
  H          = H_star/Hdim            ! Normalization for depth 
  Lx         = Lx_star/Ldim           ! Length scale
  U_Sv       = U_Sv_star / Udim       ! Velocity of gyre
  nu         = nu_star * Tdim/Ldim**2 ! Gravity
  c          = U_Sv/Fr                ! Wave speed
  g          = c**2.0_8/H             ! gravitational constant
  f0         = f0_star * Ldim/Udim
  Ro         = U_Sv_star/(beta_star*Ldim**2)
