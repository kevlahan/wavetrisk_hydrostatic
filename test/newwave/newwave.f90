module newwave_mod
  ...
  real(8), parameter :: H_star     = 6371e3 ! mean ocean depth
  ...
  real(8) :: initial_summed_mass(2), summed_mass(2), initotalmass, totalmass
...
contains
  ...
  subroutine sum_total_mass(initialgo)
    ...
    summed_mass=0.0_8
       do k = 1, zlevels
          call apply_onescale(sum_the_mass, level_start, k, 0, 0)
       end do
  ...
  subroutine init_sol(dom, i, j, zlev, offs, dims)
    ...
    if (dom%node%elts(id+1)%y .gt. 0) then
        ...
        !PRINT *, 'r2', r2
        sol(S_MASS,zlev)%data(d)%elts(id+1) = 1.0_8 + exp(-10.0_8*r2)
        !PRINT *, 'mass', sol(S_MASS,zlev)%data(d)%elts(id+1)
    else
  ...
  subroutine read_test_case_parameters(filename)
    ...
    CP_EVERY=time_end*60.0_8/dt_write !JEMF: remove checkpointing cause it is bugged
  ...
  subroutine newwave_dump(fid)
    integer fid
    write(fid) VELO_SCALE
    write(fid) iwrite
  end subroutine newwave_dump

  subroutine newwave_load(fid)
    integer fid
    read(fid) VELO_SCALE
    read(fid) iwrite
  end subroutine newwave_load
...
program newwave
  ...
  call init_main_mod()
  call read_test_case_parameters("newwave.in")
  ...
  csq = grav_accel*H
  k_tsu = 2.0_8*MATH_PI/(1e6_8/Ldim) ! Approximate wavelength of newwave: 100km
  ...
  call initialize(apply_initial_conditions, 1, set_thresholds, newwave_dump, newwave_load)

     call sum_total_mass(.True.)
     initotalmass=integrate_hex(mass_pert, level_start, 1)
     PRINT *, 'total mass integrate hex', initotalmass
  ...
  do while (time .lt. time_end)
     ...
     if (aligned) then
        ...
        ierr = writ_checkpoint(newwave_dump)
        ...
        call restart_full(set_thresholds, newwave_load)
        call barrier()
     end if

     call sum_total_mass(.False.)
     PRINT*, 'relative change in mass', abs(summed_mass-initial_summed_mass)/initial_summed_mass
          totalmass=integrate_hex(mass_pert, level_start, 1)
