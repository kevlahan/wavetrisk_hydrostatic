...
!       threshold = 0.01d0*threshold_new + 0.99d0*threshold
  ...
  subroutine initialize_thresholds_case
    ...
    ! lnorm(S_MASS,:) = ref_density * dH
    ! lnorm(S_TEMP,:) = 1d16
    ! lnorm(S_VELO,:) = Udim

    !lnorm(S_MASS,:) = 1d16
    lnorm(S_MASS,:) = ref_density * abs(max_depth) 
    lnorm(S_TEMP,:) = 1d16
    lnorm(S_VELO,:) = Udim
  ...
  subroutine initialize_dt_viscosity_case
    ...
    ! Diffusion constants
    C_sclr = 1.5d-1   ! <= 1.75e-2 for hyperdiffusion (lower than exact limit 1/6^2 = 2.8e-2 due to non-uniform grid)
  ...
  subroutine set_penal (dom, i, j, zlev, offs, dims)
    ...
    if (penalize) then
       if (etopo_coast) then
          call etopo_topography (dom, i, j, zlev, offs, dims, "penalize", npts_penal)
       else
  ...
  subroutine etopo_topography (dom, i, j, zlev, offs, dims, itype, npts)
