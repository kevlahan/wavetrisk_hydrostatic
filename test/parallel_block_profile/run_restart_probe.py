...
for label,binary in (('legacy',args.legacy),('block',args.block)):
    ...
    shutil.copy2(args.fixture/'test_checkpoint_0003.bin.zst',root)
    ...
    (root/'identity.json').write_text(json.dumps({'binary':str(binary),'sha256':digest(binary),'env':recorded},indent=2))
