...
contains
  ...
  subroutine tenlayergausshotspot_dump(fid)
    integer fid
    write(fid) VELO_SCALE
    write(fid) iwrite
  end subroutine tenlayergausshotspot_dump

  subroutine tenlayergausshotspot_load(fid)
    integer fid
    read(fid) VELO_SCALE
    read(fid) iwrite
...
program tenlayergausshotspot
  use main_mod
  use tenlayergausshotspot_mod
  implicit none
  ...
  call init_main_mod()
  call read_test_case_parameters("tenlayergausshotspot.in")
  ...
  do while (time .lt. time_end)
     ...
     if (aligned) then
        ...
        ierr = writ_checkpoint(tenlayergausshotspot_dump)
        ...
        call restart_full(set_thresholds, tenlayergausshotspot_load)
        call barrier()
     end if
