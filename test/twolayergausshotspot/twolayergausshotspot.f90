module twolayergausshotspot_mod
  use main_mod
  ...
  real(8), parameter :: Omega_star = 0.0_8 !7.29e-5 !JEMF
...
contains
  ...
  subroutine init_sol(dom, i, j, zlev, offs, dims)
    ...
    sol(S_TEMP,zlev)%data(d)%elts(id+1) = 1.0_8 + 0.01_8*exp(-100.0_8*rgrc*rgrc)
  ...
  subroutine twolayergausshotspot_dump(fid)
    integer fid
    write(fid) VELO_SCALE
    write(fid) iwrite
  end subroutine twolayergausshotspot_dump

  subroutine twolayergausshotspot_load(fid)
    integer fid
    read(fid) VELO_SCALE
    read(fid) iwrite
  end subroutine twolayergausshotspot_load
...
end module twolayergausshotspot_mod

program twolayergausshotspot
  use main_mod
  use twolayergausshotspot_mod
  implicit none
  ...
  call init_main_mod()
  call read_test_case_parameters("twolayergausshotspot.in")
  ...
  csq = grav_accel*H
  k_tsu = 2.0_8*MATH_PI/(1e6_8/Ldim) ! Approximate wavelength of twolayergausshotspot: 100km
  ...
  call initialize(apply_initial_conditions, 1, set_thresholds, twolayergausshotspot_dump, twolayergausshotspot_load)
  ...
  do while (time .lt. time_end)
     ...
     if (aligned) then
        ...
        ierr = writ_checkpoint(twolayergausshotspot_dump)
        ...
        call restart_full(set_thresholds, twolayergausshotspot_load)
        call barrier()
     end if
  ...
  call finalize()
end program twolayergausshotspot
