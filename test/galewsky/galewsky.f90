  ...
  contains
    ...
    subroutine write_and_print_step()
      ...
      !! FIX ME: include only zlevel 1
      zlev = 1
      energ1    = integrate_hex(energy,     level_start, zlev)
      energ_pot = integrate_hex(pot_energy, level_start, zlev)
    ...
    !   end do
    ! end subroutine write_grid_and_vort
    ...
    subroutine init_sol(dom, i, j, offs, dims)
      ...
      do k = 1, zlevels
         sol(S_MASS,k)%data(d)%elts(id+1) = height_fun(lon, lat)

         sol(S_VELO,k)%data(d)%elts(EDGE*id+RT+1) = proj_vel(vel_fun, dom%node%elts(id+1),   dom%node%elts(idE+1))
         sol(S_VELO,k)%data(d)%elts(DG+EDGE*id+1) = proj_vel(vel_fun, dom%node%elts(idNE+1), dom%node%elts(id+1))
         sol(S_VELO,k)%data(d)%elts(EDGE*id+UP+1) = proj_vel(vel_fun, dom%node%elts(id+1),   dom%node%elts(idN+1))
      end do
    ...
    subroutine add_sol(dom, p, i, j, offs, dims)
      ...
      do k = 1, zlevels
         sol(S_MASS,k)%data(d)%elts(id+1) = sol(S_MASS,k)%data(d)%elts(id+1) + height_fun(lon, lat)

         sol(S_VELO,k)%data(d)%elts(EDGE*id+RT+1) = sol(S_VELO,k)%data(d)%elts(EDGE*id+RT+1) + &
              proj_vel(vel_fun, dom%node%elts(id+1), dom%node%elts(idE+1))

         sol(S_VELO,k)%data(d)%elts(DG+EDGE*id+1) = sol(S_VELO,k)%data(d)%elts(DG+EDGE*id+1) + &
              proj_vel(vel_fun, dom%node%elts(idNE+1), dom%node%elts(id+1))

         sol(S_VELO,k)%data(d)%elts(EDGE*id+UP+1) = sol(S_VELO,k)%data(d)%elts(EDGE*id+UP+1) + &
              proj_vel(vel_fun, dom%node%elts(id+1), dom%node%elts(idN+1))
      end do
    end subroutine add_sol
    ...
    subroutine galewsky_dump(fid)
      integer fid
      write(fid) iwrite
    end subroutine galewsky_dump

    subroutine galewsky_load(fid)
      integer fid
      read(fid) iwrite
    end subroutine galewsky_load

    subroutine set_thresholds()
      tol_velo  = Ro_number*velo_scale*threshold**(3.0_8/2.0_8)
      tol_mass = Ro_number*velo_scale*length_scale*corolis_param*threshold**(3.0_8/2.0_8)
    end subroutine set_thresholds

  end module galewsky_mod
  ...
  program galewsky
    ...
    integer recommended_level_start, d
    ...
    recommended_level_start = write_active_per_level()
    ...
    call init_Float_Field(pv_smooth, S_MASS)
    ...
    !! FIXME: Only exporting level 1
    call export_2d(cart2sph2, (/sol(S_MASS,1), pv_smooth/), 2, 100000+min_level*1000+level_end*10, level_end, &
         (/-768, 768/), (/-384, 384/), (/2.0_8*MATH_PI, MATH_PI/), (/0.0_8, 0.0_8/))

    call finalize()
  end program galewsky
