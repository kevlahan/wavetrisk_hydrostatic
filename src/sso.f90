...
contains
  function sso_drag (dom, i, j, z_null, offs, dims)
    ...
    real(dp)                           :: lat, lon
    real(dp), dimension(1:EDGE)        :: u_av
    real(dp), dimension(2)             :: vel
    real(dp), dimension(1:zlevels)     :: N_bv, phi, psi, rho, drag_block, drag_wave, umag
    real(dp), dimension(0:zlevels)     :: z

    type(Coord)                        :: e_zonal, e_merid, e_Uav, e_U, e_V, e_W

    real(dp), parameter                :: C_d    = 2.00_dp
    real(dp), parameter                :: H_crit = 0.50_dp
    real(dp), parameter                :: G      = 0.25_dp
    real(dp), parameter                :: n_eff  = 2.40_dp
    ...
    H_peak = 3 * mu ! maximum SSO peak height
    H_env  = 2 * mu ! SSO envelope
    ...
    if (wave_drag) then
       ...
       if (circle) then ! circular mountain
          drag_wave(1) = - rho_av * N_av * (H_eff/3)**2 * sigma/mu * G  * 0.78 / rho_dz
       else
          drag_wave(1) = - rho_av * N_av * (H_eff/3)**2 * sigma/mu * G  &
    ...
    if (blocking_drag) then
       do k = 1, zlevels
          if (z(k) <= Z_block) then
             if (circle) then ! circular mountain
                drag_block(k) = - 0.5 * C_d * 4  * sigma/H_env * sqrt ((Z_block - z(k))/(z(k) + mu)) * umag(k) * 0.78 
             else
                r = sqrt ( (cos(psi(k))**2 + gamma**2 * sin(psi(k))**2) / (gamma**2 * cos(psi(k))**2 + sin(psi(k))**2) )

                drag_block(k) = - 0.5 * C_d * max (5.0_dp - 1/r**3, 0.0_dp)  * sigma/H_env &
