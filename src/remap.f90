...
contains
  ...
  subroutine find_coordinates (z_new, z_old, d, id_i)
    ...
    z_new(0) = 0.0_8
    do k = 1, zlevels
       z_old(k) = z_old(k-1) + trend(S_MASS,k)%data(d)%elts(id_i)
       z_new(k) = z_new(k-1) +   sol(S_MASS,k)%data(d)%elts(id_i)
    end do
  ...
  subroutine remap2PPM_tile (N, var_new, z_new, var_old, z_old)
    !
    ...
    logical, parameter      :: LIMIT_INTERIOR = .true., LIMIT_SLOPES=.true., NEUMANN=.false.
  ...
  subroutine remap2S (N, var_new, z_new, var_old, z_old)
    ...
    logical, parameter      :: NEUMANN = .false., LINEAR_CONTINUATION = .false., PARABOLIC_CONTINUATION = .true.
  ...
  subroutine remap2W (N, var_new, z_new, var_old, z_old)
    ...
    logical, parameter      :: LIMIT_INTERIOR = .false. ! Setting true could cause oscillations at boundary
    logical, parameter      :: LIMIT_SLOPES = .true., NEUMANN = .false. 
    ...
    if (LIMIT_INTERIOR) then
       ...
       aR(1) = var_old(1)     ! and botommost grid boxes. Note 
       aL(1) = var_old(1)     ! that even if for NEUMANN boundary 
    ...
    do k = 1, N
       if (LIMIT_INTERIOR) then               ! Constrain parabolic
          ...
          if (deltaR*deltaL < Zero) then
             deltaR = Zero
             deltaL = Zero
          elseif (abs(deltaR) > abs(cffL)) then
             deltaR = cffL
          elseif (abs(deltaL) > abs(cffR)) then
             deltaL = cffR
          endif
          aR(k) = var_old(k) + deltaR
          aL(k) = var_old(k) - deltaL
  ...
  subroutine remap4_tile (N, var_new, z_new, var_old, z_old)
