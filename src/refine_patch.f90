...
contains
  ...
  logical function check_child_required (dom, p, c)
    ! Determines where child (finer grid) is required based on mask value for the cell
    ! child is required if cell is in the adjacent zone
    implicit none
    type(Domain) :: dom
    integer      :: p, c
    ...
    do j0 = st + 1, PATCH_SIZE/2 + en
       ...
       do i0 = st + 1, PATCH_SIZE/2 + en
          ...
          ! Child is required because parent node is in adjacent zone
          ! or flux at parent edge is obtained by restriction from finer grid
