...
contains
  ...
  subroutine flux_grad_scalar (dom, i, j, zlev, offs, dims)
    ...
    h_tflux(EDGE*id+RT+1) = h_tflux(EDGE*id+RT+1) &
         - viscosity * (temp(idE+1) - temp(id+1))   * dom%pedlen%elts(EDGE*id+RT+1)/dom%len%elts(EDGE*id+RT+1)

    h_tflux(EDGE*id+DG+1) = h_tflux(EDGE*id+DG+1) &
         - viscosity * (temp(id+1)  - temp(idNE+1)) * dom%pedlen%elts(EDGE*id+DG+1)/dom%len%elts(EDGE*id+DG+1)

    h_tflux(EDGE*id+UP+1) = h_tflux(EDGE*id+UP+1) &
         - viscosity * (temp(idN+1) - temp(id+1))   * dom%pedlen%elts(EDGE*id+UP+1)/dom%len%elts(EDGE*id+UP+1)
  ...
  subroutine diffuse_momentum (dom, i, j, zlev, offs, dims)
    ...
    dvelo(id*EDGE+RT+1) = dvelo(id*EDGE+RT+1) &
         + viscosity * (divu(idE+1)-divu(id+1) + vort(id*TRIAG+LORT+1)-vort(idS*TRIAG+UPLT+1)) &
         * dom%len%elts(id*EDGE+RT+1)/dom%pedlen%elts(id*EDGE+RT+1)

    dvelo(id*EDGE+DG+1) = dvelo(id*EDGE+DG+1) &
         + viscosity * (divu(id+1)-divu(idNE+1) + vort(id*TRIAG+LORT+1)-vort(id*TRIAG+UPLT+1)) &
         * dom%len%elts(id*EDGE+DG+1)/dom%pedlen%elts(id*EDGE+DG+1)

    dvelo(id*EDGE+UP+1) = dvelo(id*EDGE+UP+1) &
         + viscosity * (divu(idN+1)-divu(id+1)  + vort(idW*TRIAG+LORT+1)-vort(id*TRIAG+UPLT+1)) &
