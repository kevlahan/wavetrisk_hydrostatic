module topo_grid_descriptor_mod

#ifdef MPI
  use mpi_f08
#endif
  use netcdf
  
  use kind_mod,   only : dp
  use shared_mod, only : N_BDRY, LORT, UPLT, TRIAG, grav_accel, max_level, radius, z_null

  use arch_mod,       only : MPI_DP, MPI_IN, comm, glo_id, rank
  use comm_mpi_mod,   only : sum_int, update_bdry
  use domain_mod,     only : Domain, grid, topography, idx
  use domain_ops_mod, only : apply_onescale
  use geom_mod,       only : cart2sph

  implicit none
  
  private
  public :: assign_height, write_grid_coords

  integer                               :: icol, loc_size, ncol
  integer, dimension(:),    allocatable :: grid_dom
  integer, dimension(:),    allocatable :: grid_id
  integer, dimension(:),    allocatable :: loc_ids
  integer, dimension(:),    allocatable :: loc_dom
  real(dp), dimension(:),   allocatable :: loc_area
  real(dp), dimension(:),   allocatable :: loc_center_lat
  real(dp), dimension(:),   allocatable :: loc_center_lon
  real(dp), dimension(:),   allocatable :: phi_s
  real(dp), dimension(:,:), allocatable :: loc_corner_lat
  real(dp), dimension(:,:), allocatable :: loc_corner_lon

  !
  !  DATE CODED:  November 2023
  !  DESCRIPTION:
  !
  !  Generates smoothed topography data for WAVETRISK from NCAR topography NetCDF files using cube_to_target
  !  program that remaps topography data from cubed-sphere grid to target grid (non-adaptive WAVETRISK grid at max_level)
  !  grid using rigorous remapping (Lauritzen, Nair and Ullrich, 2010, J. Comput. Phys.).
  !
  !  Workflow
  !
  !  First step: pre-processing of coordinate data
  !
  !                                call write_grid_coords
  !
  !  to produce a grid descriptor file (e.g. J08_topo_grid.nc) for ESMF/SCRIP software in NetCDF file format
  !  for the hexagons on a given non-adaptive WAVETRISK grid (e.g. the grid corresponding to the desired max_level).
  !
  !  Second step:
  !
  !  Use the external program
  !
  !                                    cube_to_target
  !
  !  to generate the NetCDF file that provides the surface geopotential phi_S = z/g corresponding to the hexagons 
  !  saved in Step 1.  It is practical to use a script to specify appropriate parameters e.g. J08.sh.
  !                         
  !  Third step: each time you run the test case:
  !
  !                               call assign_height (fname)
  !
  !  to read in the .nc file generated in Step 2 to assign the topogragraphy data to the float field topography,
  !  which must have the same max_level and domain configuration as the WAVETRISK grid that generated the data in Step 1.
  !
  !  Author: Peter Hjort Lauritzen (pel@ucar.edu)
  !  Modified for inclusion WAVETRISK by Nicholas Kevlahan (kevlahan@mcmaster.ca) 2023-10
  !


contains


  subroutine write_grid_coords (topo_desc)
    ! Find grid coordinates on each domain at finest level over entire grid
    ! saves results to netcdf coordinate file
    
    implicit none
    
    character(255)        :: topo_desc

    integer               :: i, l
    integer               :: grid_size          ! number of nodes
    integer, parameter    :: grid_corners = 6   ! hexagons
    integer,  allocatable :: grid_dom(:)
    integer,  allocatable :: grid_imask(:)        
    integer,  allocatable :: grid_id(:)           
    real (8), allocatable :: grid_area(:)        
    real (8), allocatable :: grid_center_lat(:)
    real (8), allocatable :: grid_center_lon(:)
    real (8), allocatable :: grid_corner_lat(:,:)
    real (8), allocatable :: grid_corner_lon(:,:)

    ! Compute topography on finest grid
    l = max_level

    loc_size = 0
    call apply_onescale (count_nodes, l, z_null, 0, 1)

    allocate (loc_dom(1:loc_size), loc_ids(1:loc_size))
    allocate (loc_area(1:loc_size), loc_center_lat(1:loc_size), loc_center_lon(1:loc_size))
    allocate (loc_corner_lat(1:grid_corners,1:loc_size), loc_corner_lon(1:grid_corners,1:loc_size))

    icol = 0
    call apply_onescale (grid_coords, l, z_null, 0, 1)

#ifdef MPI
    grid_size = sum_int (loc_size)
#else
    grid_size = loc_size
#endif
    allocate (grid_imask(1:grid_size)); grid_imask = 1
    allocate (grid_dom(1:grid_size), grid_id(1:grid_size))
    allocate (grid_area(1:grid_size), grid_center_lat(1:grid_size), grid_center_lon(1:grid_size))
    allocate (grid_corner_lat(1:grid_corners,1:grid_size), grid_corner_lon(1:grid_corners,1:grid_size))

#ifdef MPI
    call MPI_Gather (loc_dom, loc_size, MPI_IN, grid_dom, loc_size, MPI_IN, 0, comm)
    call MPI_Gather (loc_ids, loc_size, MPI_IN, grid_id,  loc_size, MPI_IN, 0, comm)

    call MPI_Gather (loc_area, loc_size, MPI_DP, grid_area, loc_size, MPI_DP, 0, comm)

    call MPI_Gather (loc_center_lat, loc_size, MPI_DP, grid_center_lat, loc_size, MPI_DP, 0, comm)

    call MPI_Gather (loc_center_lon, loc_size, MPI_DP, grid_center_lon, loc_size, MPI_DP, 0, comm)

    do i = 1, grid_corners
       call MPI_Gather (loc_corner_lat(i,:), loc_size, MPI_DP, grid_corner_lat(i,:), loc_size, MPI_DP, 0, comm)
       call MPI_Gather (loc_corner_lon(i,:), loc_size, MPI_DP, grid_corner_lon(i,:), loc_size, MPI_DP, 0, comm)
    end do
#else
    grid_dom = loc_dom; grid_id = loc_ids; grid_area = loc_area; grid_center_lat = loc_center_lat; 
    grid_center_lon = loc_center_lon; grid_corner_lat = loc_corner_lat; grid_corner_lon = loc_corner_lon
#endif

    if (rank==0) call wrt_esmf_rll
    
  contains
    
    subroutine wrt_esmf_rll
      !
      ! Write netCDF grid descriptor file for ESMF remapping.
      !
      
      implicit none

      !-----------------------------------------------------------------------
      ! NetCDF identifiers
      !-----------------------------------------------------------------------
      integer :: ncstat
      integer :: nc_grid_id
      integer :: nc_gridsize_id
      integer :: nc_gridcorn_id
      integer :: nc_gridrank_id
      integer :: nc_griddims_id
      integer :: nc_grddom_id
      integer :: nc_grdid_id
      integer :: nc_grdarea_id
      integer :: nc_grdcntrlat_id
      integer :: nc_grdcntrlon_id
      integer :: nc_grdcrnrlat_id
      integer :: nc_grdcrnrlon_id
      integer :: nc_grdimask_id

      integer :: nc_dims2_id(2)
      integer :: grid_dims(1)

      !-----------------------------------------------------------------------
      ! Create the NetCDF dataset
      !-----------------------------------------------------------------------
      ncstat = nf90_create( &
           trim(topo_desc)//'.nc', &
           NF90_CLOBBER,           &
           nc_grid_id)
      call handle_err(ncstat)

      ncstat = nf90_put_att( &
           nc_grid_id, &
           NF90_GLOBAL, &
           'title', &
           trim(topo_desc))
      call handle_err(ncstat)

      !-----------------------------------------------------------------------
      ! Define dimensions
      !-----------------------------------------------------------------------
      ncstat = nf90_def_dim( &
           nc_grid_id, &
           'grid_size', &
           grid_size, &
           nc_gridsize_id)
      call handle_err(ncstat)

      ncstat = nf90_def_dim( &
           nc_grid_id, &
           'grid_corners', &
           grid_corners, &
           nc_gridcorn_id)
      call handle_err(ncstat)

      ncstat = nf90_def_dim( &
           nc_grid_id, &
           'grid_rank', &
           1, &
           nc_gridrank_id)
      call handle_err(ncstat)

      !-----------------------------------------------------------------------
      ! Define variables
      !-----------------------------------------------------------------------

      ! Grid dimensions
      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_dims', &
           NF90_INT, &
           [nc_gridrank_id], &
           nc_griddims_id)
      call handle_err(ncstat)

      ! Domain number
      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_dom', &
           NF90_INT, &
           [nc_gridsize_id], &
           nc_grddom_id)
      call handle_err(ncstat)

      ! Grid-point identifier
      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_id', &
           NF90_INT, &
           [nc_gridsize_id], &
           nc_grdid_id)
      call handle_err(ncstat)

      ! Grid-cell area
      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_area', &
           NF90_DOUBLE, &
           [nc_gridsize_id], &
           nc_grdarea_id)
      call handle_err(ncstat)

      ncstat = nf90_put_att( &
           nc_grid_id, &
           nc_grdarea_id, &
           'units', &
           'radians^2')
      call handle_err(ncstat)

      ! Grid-centre latitude
      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_center_lat', &
           NF90_DOUBLE, &
           [nc_gridsize_id], &
           nc_grdcntrlat_id)
      call handle_err(ncstat)

      ncstat = nf90_put_att( &
           nc_grid_id, &
           nc_grdcntrlat_id, &
           'units', &
           'radians')
      call handle_err(ncstat)

      ! Grid-centre longitude
      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_center_lon', &
           NF90_DOUBLE, &
           [nc_gridsize_id], &
           nc_grdcntrlon_id)
      call handle_err(ncstat)

      ncstat = nf90_put_att( &
           nc_grid_id, &
           nc_grdcntrlon_id, &
           'units', &
           'radians')
      call handle_err(ncstat)

      ! Grid mask
      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_imask', &
           NF90_INT, &
           [nc_gridsize_id], &
           nc_grdimask_id)
      call handle_err(ncstat)

      ncstat = nf90_put_att( &
           nc_grid_id, &
           nc_grdimask_id, &
           'units', &
           'unitless')
      call handle_err(ncstat)

      !-----------------------------------------------------------------------
      ! Define corner-coordinate variables
      !
      ! The NetCDF Fortran interface reverses dimension order relative to the
      ! order displayed by C-oriented tools such as ncdump. Passing
      ! [grid_corners, grid_size] corresponds to a Fortran array declared as
      ! grid_corner_lat(grid_corners, grid_size).
      !-----------------------------------------------------------------------
      nc_dims2_id = [nc_gridcorn_id, nc_gridsize_id]

      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_corner_lat', &
           NF90_DOUBLE, &
           nc_dims2_id, &
           nc_grdcrnrlat_id)
      call handle_err(ncstat)

      ncstat = nf90_put_att( &
           nc_grid_id, &
           nc_grdcrnrlat_id, &
           'units', &
           'radians')
      call handle_err(ncstat)

      ncstat = nf90_def_var( &
           nc_grid_id, &
           'grid_corner_lon', &
           NF90_DOUBLE, &
           nc_dims2_id, &
           nc_grdcrnrlon_id)
      call handle_err(ncstat)

      ncstat = nf90_put_att( &
           nc_grid_id, &
           nc_grdcrnrlon_id, &
           'units', &
           'radians')
      call handle_err(ncstat)

      !-----------------------------------------------------------------------
      ! End define mode
      !-----------------------------------------------------------------------
      ncstat = nf90_enddef(nc_grid_id)
      call handle_err(ncstat)

      !-----------------------------------------------------------------------
      ! Write grid data
      !-----------------------------------------------------------------------
      write (6, '(/,a)', advance='no') &
           'Writing NCAR topography grid descriptor file ' // &
           trim(topo_desc) // '.nc ...'

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_griddims_id, &
           grid_dims)
      call handle_err(ncstat)

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_grdimask_id, &
           grid_imask)
      call handle_err(ncstat)

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_grddom_id, &
           grid_dom)
      call handle_err(ncstat)

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_grdid_id, &
           grid_id)
      call handle_err(ncstat)

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_grdarea_id, &
           grid_area)
      call handle_err(ncstat)

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_grdcntrlat_id, &
           grid_center_lat)
      call handle_err(ncstat)

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_grdcntrlon_id, &
           grid_center_lon)
      call handle_err(ncstat)

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_grdcrnrlat_id, &
           grid_corner_lat)
      call handle_err(ncstat)

      ncstat = nf90_put_var( &
           nc_grid_id, &
           nc_grdcrnrlon_id, &
           grid_corner_lon)
      call handle_err(ncstat)

      ncstat = nf90_close(nc_grid_id)
      call handle_err(ncstat)

      write (6, '(a)') ' finished writing file descriptor data file.'
      write (6, '(a,/)') &
           'Use cube_to_target to generate corresponding surface geopotential .nc file.'

    end subroutine wrt_esmf_rll
  end subroutine write_grid_coords
  

  subroutine count_nodes (dom, i, j, zlev, offs, dims)
    ! Count number of nodes

    implicit none
    
    type(Domain), intent(inout) :: dom
    integer,      intent(in)    :: i, j, zlev
    integer,      intent(in)    :: offs(N_BDRY+1)
    integer,      intent(in)    :: dims(2,N_BDRY+1)

    loc_size = loc_size + 1
  end subroutine count_nodes
  

  subroutine grid_coords (dom, i, j, zlev, offs, dims)
    ! Set grid coordinates

    implicit none
    
    type(Domain), intent(inout) :: dom
    integer,      intent(in)    :: i, j, zlev
    integer,      intent(in)    :: offs(N_BDRY+1)
    integer,      intent(in)    :: dims(2,N_BDRY+1)

    integer  :: d, id, id_i, idS, idSW, idW
    real(dp) :: lat, lon

    d = dom%id + 1
    id = idx (i, j, offs, dims)
    id_i = id + 1

    idW  = idx (i-1, j,   offs, dims)
    idSW = idx (i-1, j-1, offs, dims)
    idS  = idx (i,   j-1, offs, dims)

    icol = icol + 1

    loc_ids(icol) = id                   ! id of node
    loc_dom(icol) = glo_id(rank+1,d) + 1 ! global domain label associated to node id

    loc_area(icol) = 1 / dom%areas%elts(id_i)%hex_inv / radius**2 ! hexagon area (unit sphere)

    call cart2sph (dom%node%elts(id_i), lon, lat) ! longitude and latitude coordinates of node in radians
    loc_center_lat(icol) = lat
    loc_center_lon(icol) = lon

    call cart2sph (dom%ccentre%elts(TRIAG*id+LORT+1), lon, lat)    
    loc_corner_lat(1,icol) = lat
    loc_corner_lon(1,icol) = lon

    call cart2sph (dom%ccentre%elts(TRIAG*id+UPLT+1), lon, lat)    
    loc_corner_lat(2,icol) = lat
    loc_corner_lon(2,icol) = lon

    call cart2sph (dom%ccentre%elts(TRIAG*idW+LORT+1), lon, lat)    
    loc_corner_lat(3,icol) = lat
    loc_corner_lon(3,icol) = lon

    call cart2sph (dom%ccentre%elts(TRIAG*idSW+UPLT+1), lon, lat)    
    loc_corner_lat(4,icol) = lat
    loc_corner_lon(4,icol) = lon

    call cart2sph (dom%ccentre%elts(TRIAG*idSW+LORT+1), lon, lat)    
    loc_corner_lat(5,icol) = lat
    loc_corner_lon(5,icol) = lon

    call cart2sph (dom%ccentre%elts(TRIAG*idS+UPLT+1), lon, lat)    
    loc_corner_lat(6,icol) = lat
    loc_corner_lon(6,icol) = lon
  end subroutine grid_coords
  

  subroutine assign_height (fname)
    ! Reads netcdf geopotential data and saves it to the variable topography
    
    implicit none
    
    character(*), intent(in) :: fname

    integer :: l

    ! Compute topography on finest grid
    l = max_level

    if (rank == 0) write (6,'(/,a,a,a)',advance="no") "Reading NCAR topography data ", trim(fname)//".nc ..."

    call read_geopotential

    call apply_onescale (cal_assign_height, l, z_null, 0, 1)

    topography%bdry_uptodate = .false.
    call update_bdry (topography, max_level, 978)

    if (rank == 0) write (6,'(a,/)') " finished reading topography data."
    
  contains
    
    subroutine read_geopotential
      ! Reads netcdf geopotential data onto a single core and saves the data as its wavelet coefficients
      
      implicit none
      
      integer :: dimid, domid, idxid, ncid, status,  phisid
      !********************************************
      !
      ! Get dimension on rank 0
      !
      !********************************************
      if (rank == 0) then
         status = nf90_open(trim(fname)//".nc", NF90_NOWRITE, ncid)
         if (status /= NF90_NOERR) call handle_err(status)

         status = nf90_inq_dimid(ncid, "ncol", dimid)
         if (status /= NF90_NOERR) call handle_err(status)

         status = nf90_inquire_dimension(ncid, dimid, len=ncol)
         if (status /= NF90_NOERR) call handle_err(status)
      end if

#ifdef MPI
      call MPI_Bcast (ncol, 1, MPI_IN, 0, comm)
#endif
      allocate (grid_dom(1:ncol), grid_id(1:ncol), phi_s(1:ncol))

      !********************************************
      !
      !  Read variables on rank 0
      !   - all are dimension(ncol)
      !
      !********************************************
      if (rank == 0) then
         status = nf90_inq_varid(ncid, 'dom', domid)
         if (status /= NF90_NOERR) call handle_err(status)

         status = nf90_get_var(ncid, domid, grid_dom)
         if (status /= NF90_NOERR) call handle_err(status)

         status = nf90_inq_varid(ncid, 'idx', idxid)
         if (status /= NF90_NOERR) call handle_err(status)

         status = nf90_get_var(ncid, idxid, grid_id)
         if (status /= NF90_NOERR) call handle_err(status)

         status = nf90_inq_varid(ncid, 'PHIS', phisid)
         if (status /= NF90_NOERR) call handle_err(status)

         status = nf90_get_var(ncid, phisid, phi_s)
         if (status /= NF90_NOERR) call handle_err(status)
      end if

#ifdef MPI
      call MPI_Bcast (grid_dom, ncol, MPI_IN, 0, comm)
      call MPI_Bcast (grid_id,  ncol, MPI_IN, 0, comm)
      call MPI_Bcast (phi_s,    ncol, MPI_DP, 0, comm)
#endif
      
    end subroutine read_geopotential
  end subroutine assign_height
  

  subroutine cal_assign_height (dom, i, j, zlev, offs, dims)
    ! Assign topography to multiscale float field variable topography
    
    implicit none
    
    type(Domain), intent(inout) :: dom
    integer,      intent(in)    :: i, j, zlev
    integer,      intent(in)    :: offs(N_BDRY+1)
    integer,      intent(in)    :: dims(2,N_BDRY+1)

    integer :: d, id, d_glo, icol

    d = dom%id + 1

    d_glo = glo_id(rank+1,d) + 1 ! global domain
    do icol = 1, ncol
       if (grid_dom(icol) == d_glo) then ! data is for this domain
          id = grid_id(icol) + 1
          topography%data(d)%elts(id) = phi_s(icol) / grav_accel
       end if
    end do
  end subroutine cal_assign_height
  

  subroutine handle_err(status)
    use netcdf
    implicit none

    integer, intent(in) :: status

    if (status /= NF90_NOERR) then
       write (*, '(a)') trim(nf90_strerror(status))
       error stop
    end if
  end subroutine handle_err

  
end module topo_grid_descriptor_mod
