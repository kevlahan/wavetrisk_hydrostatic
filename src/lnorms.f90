...
contains
  ...
  subroutine cal_lnorm_trend (scaling, order)
    ! Calculates l norm of a float_field
    implicit none
    ...
    do k = 1, zmax
       do l = level_start, level_end
          select case (order)
          case ("1")
             call apply_onescale (l1_scalar_trend, l, k, 0, 1)
             call apply_onescale (l1_velo_trend,   l, k, 0, 0)
          case ("2")
             call apply_onescale (l2_scalar_trend, l, k, 0, 1)
             call apply_onescale (l2_velo_trend,   l, k, 0, 0)
          case ("inf")
             call apply_onescale (linf_scalar_trend, l, k, 0, 1)
             call apply_onescale (linf_velo_trend,   l, k, 0, 0)
          end select
       end do
  ...
  end subroutine cal_lnorm_trend

  subroutine l1_scalar_trend (dom, i, j, zlev, offs, dims)
    implicit none
    type(Domain)                   :: dom
    integer                        :: i, j, zlev
    integer, dimension(N_BDRY+1)   :: offs
  ...
  end subroutine l1_scalar_trend

  subroutine l1_velo_trend (dom, i, j, zlev, offs, dims)
    implicit none
    type(Domain)                   :: dom
    integer                        :: i, j, zlev
    integer, dimension(N_BDRY+1)   :: offs
    ...
    do e = 1, EDGE
       id_e = EDGE*id+e
       if (dom%mask_e%elts(id_e) >= ADJZONE) then
          lnorm(S_VELO,zlev) = lnorm(S_VELO,zlev) + abs (trend(S_VELO,zlev)%data(d)%elts(id_e))
       end if
    end do
  end subroutine l1_velo_trend

  subroutine l2_scalar_trend (dom, i, j, zlev, offs, dims)
    implicit none
    type(Domain)                   :: dom
    integer                        :: i, j, zlev
    integer, dimension(N_BDRY+1)   :: offs
  ...
  end subroutine l2_scalar_trend

  subroutine l2_velo_trend (dom, i, j, zlev, offs, dims)
    implicit none
    type(Domain)                   :: dom
    integer                        :: i, j, zlev
    integer, dimension(N_BDRY+1)   :: offs
    ...
    do e = 1, EDGE
       id_e = EDGE*id+e
       if (dom%mask_e%elts(id_e) >= ADJZONE) then
          lnorm(S_VELO,zlev) = lnorm(S_VELO,zlev) + trend(S_VELO,zlev)%data(d)%elts(id_e)**2
       end if
    end do
  end subroutine l2_velo_trend

  subroutine linf_scalar_trend (dom, i, j, zlev, offs, dims)
    implicit none
    type(Domain)                   :: dom
    integer                        :: i, j, zlev
    integer, dimension(N_BDRY+1)   :: offs
  ...
  end subroutine linf_scalar_trend

  subroutine linf_velo_trend (dom, i, j, zlev, offs, dims)
    implicit none
    type(Domain)                   :: dom
    integer                        :: i, j, zlev
    integer, dimension(N_BDRY+1)   :: offs
    ...
    do e = 1, EDGE
       id_e = EDGE*id+e
       if (dom%mask_e%elts(id_e) >= ADJZONE) then
          lnorm(S_VELO,zlev) = max (lnorm(S_VELO,zlev), abs (trend(S_VELO,zlev)%data(d)%elts(id_e)))
       end if
    end do
  end subroutine linf_velo_trend
