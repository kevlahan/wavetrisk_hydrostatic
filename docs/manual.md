...
>[3.3 Running a test case](#markdown-header-33-Running-a-test-case)  
...
[9. VTK data analysis](#markdown-header-9-VTK-data-analysis) 
...
The standard compiler is `gfortran`. 

`netcdf`, `lapack` and `mpi` are also required (e.g. `openmpi` or `mpich`). `fftw` is required for the analysis test case `spherical_harmonics`.

<a id="markdown-header-21-load-balancing"></a>
### 2.1 Load balancing   
WAVETRISK automatically rebalances the computational load statically at each checkpoint using a simple next fit algorithm.   

<a id="markdown-header-22-optimization"></a>
### 2.2 Optimization  
Default is optimized, with `OPTIM=2` and `arch=native`.  There are two debug options: `DEBUG=check` provides extensive debugging checks on generic machines. `DEBUG=asan` includes `-fsanitize=address`, which is adss an Address Sanitizer to catch runtime memory errors like out-of-bounds array access, stack/heap overflows, and use-after-free bugs with detailed reports and line numbers.
...
### 2.4 Examples   
To compile the `climate` test case with `J5` (2°) as the coarsest grid resolution:
<pre>
<code>
...
The code checkpoints the data at an interval defined by the test case.  The full state of the simulation is saved by `io_mod/dump_adapt_mpi` and read in again by `io_mod/load_adapt_mpi`.
...
Data is written for post processing in `.vtk` format by the routine `io_vtk_mod/write_and_export` at an interval defined by the test case. Only significant cells are saved (i.e. cells in the active zone plus adjacent zone). These files can be analyzed using the python routines described in [VTK data analysis](#markdown-header-9-VTK-data-analysis) and visualized using `paraview`.

<a id="markdown-header-33-Running-a-test-case"></a>
### 3.3 Running a test case
1. To run test case climate with input file `climate.in` on a local machine with shared memory 160 cores
<pre>
<code>
    mpirun -n 160 ./climate HS.in
</code>
</pre>
...
<code> 
#!/bin/bash                                                                     
#SBATCH --job-name=J5J8Z30                                                      
#SBATCH --partition=nk                                                          
#SBATCH --ntasks=160                                                            
#SBATCH --time=30-00:00                                                                                                            
#SBATCH --output=climate-%j.out                                                 
module purge; module load gcc mpich netlib-lapack netcdf-fortran
srun ./climateJ5 climate.in
</code>
</pre>
...
<a id="markdown-header-9-VTK-data-analysis"></a>
## 9. VTK data analysis
An extensive set of python codes are provided in `~/post/paraview` for performing various conversions (e.g., from `vtk` to `vti` or `vtp` format), for constructing a 3D data set from the layer data and performing statistical analysis using the `vtk` data files. For example, `xyz2lonlat.py` computes the longitude-latitude projection from the spherical data, while `xyz2lonlat_layers.py` is a parallel version to do the conversion for all layers.